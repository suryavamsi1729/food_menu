...
const FoodDetailPage = () => {
  ...
  const { savedCount, addRecipe, isSaved } = useSavedRecipes();
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-10 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-10 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex gap-3">
              ...
              <span
                className={`rounded-full px-3 py-1 text-sm font-medium ${
                  menuItem.isVeg ? 'bg-green-500/15 text-green-500' : 'bg-red-500/15 text-red-500'
                }`}
              >
                {menuItem.isVeg ? (
                  <>
                    <Leaf size={12} className="inline-block mr-1" /> Veg
                  </>
                ) : (
                  'Non-Veg'
                )}
            ...
            <h1 className="text-5xl font-bold text-text">{menuItem.name}</h1>
            ...
            <p className="text-lg leading-8 text-text-secondary">{menuItem.fullDescription}</p>
          </div>
        </section>
